...
import warnings as wa # no individual installation because is part of the Python Standard Library
import pandas as pd
...
from sklearn.metrics import confusion_matrix, multilabel_confusion_matrix, classification_report, accuracy_score, precision_score, recall_score, f1_score, cohen_kappa_score
...
repositories = 'Please, site GitHub (https://github.com/Nicolas-Radomski/GenomicBasedClassification) and/or Docker Hub (https://hub.docker.com/r/nicolasradomski/genomicbasedclassification).'
...
acknowledgements = "Many thanks to Andrea De Ruvo and Adriano Di Pasquale for our discussions aimed at improving the algorithm."

# set the version
version = "1.0 (released in March 2025)"
...
message_traceback = 'The traceback level has been set to ' + str(sys.tracebacklimit)
...
if args.subcommand == 'modeling':
	...
	if lst_mutations == lst_phenotypes: 
		message_sample_identifiers = "The sample identifiers were confirmed as identical between the input files of mutations and phenotypes/datasets"
		print (message_sample_identifiers)
	else: 
		message_sample_identifiers = "The sample identifiers were confirmed as not identical between the input files of mutations and phenotypes/datasets"
	...
	dectection_train = all(element >= LIMIT for element in count_classes_df['training'].tolist())
	### detect small number of samples in the testing dataset
	dectection_test = all(element >= LIMIT for element in count_classes_df['testing'].tolist())
	### check the minimal quantity of samples per class
	if (dectection_train == True) and (dectection_test == True):
	...
	elif (dectection_train == False) or (dectection_test == False):
	...
	## initialize tqdm progress bar only after the model is ready with leave=False to avoid second bar after completion
	with tqdm_joblib(tqdm(desc="Model building progress", total=param_combinations, position=0, leave=False, dynamic_ncols=True)) as progress_bar:
		### fit the models
	...
	# retrieve the combinaisons of tested parameters and corresponding scores
	## combinaisons of tested parameters
	...
	# calculate the class dependant metrics
	'''
	support = TP + FN
	...
	Cohen's kappa = (2*(TP*TN-FN*FP)) / ((TP+FP)*(FP+TN)*(TP+FN)*(FN+TN))
	'''
	## from the training dataset
	metrics_classes_train_df['support'] = round((metrics_classes_train_df.TP + metrics_classes_train_df.FN), digits)
	metrics_classes_train_df['accuracy'] = round((metrics_classes_train_df.TP + metrics_classes_train_df.TN) / (metrics_classes_train_df.TP + metrics_classes_train_df.FP + metrics_classes_train_df.FN + metrics_classes_train_df.TN), digits)
	metrics_classes_train_df['sensitivity'] = round((metrics_classes_train_df.TP) / (metrics_classes_train_df.TP + metrics_classes_train_df.FN), digits)
	metrics_classes_train_df['specificity'] = round((metrics_classes_train_df.TN) / (metrics_classes_train_df.TN + metrics_classes_train_df.FP), digits)
	metrics_classes_train_df['precision'] = round((metrics_classes_train_df.TP) / (metrics_classes_train_df.TP + metrics_classes_train_df.FP), digits)
	metrics_classes_train_df['recall'] = round((metrics_classes_train_df.TP) / (metrics_classes_train_df.TP + metrics_classes_train_df.FN), digits)
	metrics_classes_train_df['f1-score'] = round(2 * ((metrics_classes_train_df.precision) * (metrics_classes_train_df.recall)) / ((metrics_classes_train_df.precision) + (metrics_classes_train_df.recall)), digits)
	metrics_classes_train_df['Cohen kappa'] = round(((2 * (metrics_classes_train_df.TP * metrics_classes_train_df.TN - metrics_classes_train_df.FN * metrics_classes_train_df.FP)) / ((metrics_classes_train_df.TP + metrics_classes_train_df.FP) * (metrics_classes_train_df.FP + metrics_classes_train_df.TN) + (metrics_classes_train_df.TP + metrics_classes_train_df.FN) * (metrics_classes_train_df.FN + metrics_classes_train_df.TN))), digits)
	## from the testing dataset
	metrics_classes_test_df['support'] = round((metrics_classes_test_df.TP + metrics_classes_test_df.FN), 2)
	metrics_classes_test_df['accuracy'] = round((metrics_classes_test_df.TP + metrics_classes_test_df.TN) / (metrics_classes_test_df.TP + metrics_classes_test_df.FP + metrics_classes_test_df.FN + metrics_classes_test_df.TN), digits)
	metrics_classes_test_df['sensitivity'] = round((metrics_classes_test_df.TP) / (metrics_classes_test_df.TP + metrics_classes_test_df.FN), digits)
	metrics_classes_test_df['specificity'] = round((metrics_classes_test_df.TN) / (metrics_classes_test_df.TN + metrics_classes_test_df.FP), digits)
	metrics_classes_test_df['precision'] = round((metrics_classes_test_df.TP) / (metrics_classes_test_df.TP + metrics_classes_test_df.FP), digits)
	metrics_classes_test_df['recall'] = round((metrics_classes_test_df.TP) / (metrics_classes_test_df.TP + metrics_classes_test_df.FN), digits)
	metrics_classes_test_df['f1-score'] = round(2 * ((metrics_classes_test_df.precision) * (metrics_classes_test_df.recall)) / ((metrics_classes_test_df.precision) + (metrics_classes_test_df.recall)), digits)
	metrics_classes_test_df['Cohen kappa'] = round(((2 * (metrics_classes_test_df.TP * metrics_classes_test_df.TN - metrics_classes_test_df.FN * metrics_classes_test_df.FP)) / ((metrics_classes_test_df.TP + metrics_classes_test_df.FP) * (metrics_classes_test_df.FP + metrics_classes_test_df.TN) + (metrics_classes_test_df.TP + metrics_classes_test_df.FN) * (metrics_classes_test_df.FN + metrics_classes_test_df.TN))), digits)

	# calculate the global metrics
	## from the training dataset
	accuracy_train = accuracy_score(y_train, y_pred_train)
	sensitivity_train = sensitivity_score(y_train, y_pred_train, average='macro')
	specificity_train = specificity_score(y_train, y_pred_train, average='macro')
	...
	f1_score_train = f1_score(y_train, y_pred_train, average='macro', zero_division = 0)
	cohen_kappa_train = cohen_kappa_score(y_train, y_pred_train)
	## from the testing dataset
	accuracy_test = accuracy_score(y_test, y_pred_test)
	sensitivity_test = sensitivity_score(y_test, y_pred_test, average='macro')
	specificity_test = specificity_score(y_test, y_pred_test, average='macro')
	...
	f1_score_test = f1_score(y_test, y_pred_test, average='macro', zero_division = 0)
	...
	# combine phenotypes and datasets to potentially use it as futur input
	## select columns of interest
	simplified_train_df = combined_train_df.iloc[:,0:2]
	simplified_test_df = combined_test_df.iloc[:,0:2]
	## add a column
	...
	log_file.writelines(["########################\n####### versions #######\n########################\n"])
	log_file.writelines("GenomicBasedClassification: " + version + "\n")
	...
	if INPUTPATH_ENCODED_CLASSES == None:
		message_encoded_classes = "The encoded classes were not provided for a model other than the XGB model"
		print(message_encoded_classes)		
	else:
		message_encoded_classes = "The encoded classes were provided for the XGB model"
		print(message_encoded_classes)
	...
	detected_model = str(loaded_model).split('(', 1)[0]
	...
	if (INPUTPATH_ENCODED_CLASSES != None) and (detected_model != "XGBClassifier"):
		...
		message_compatibility_model_classes = "The XGB classifier of the provided best model required to provide encoded classes"
		raise Exception(message_compatibility_model_classes)
		exit()
	else:
		message_compatibility_model_classes = "The classifier of the provided best model and potentially encoded classes were properly provided"
